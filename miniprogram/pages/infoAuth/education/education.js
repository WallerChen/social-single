      ...
      success: async (res) => {
        ...
        try {
          if (res.tapIndex === 0) { // 拍照
            filePath = await util.chooseCamera(3 - this.data.images.length)
          } else if (res.tapIndex === 1) { // 相册
            filePath = await util.chooseAlbum(3 - this.data.images.length)
          } else { // 聊天记录
            filePath = await util.chooseMessageFile(3 - this.data.images.length)
          }
